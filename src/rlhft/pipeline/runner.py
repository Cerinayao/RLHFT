...
def run(
    ...
    *,
    ...
    if cfg.make_plots:
        ...
        matplotlib_sections.append((f"{cfg.trading.col_a}/{cfg.trading.col_b} — Rule vs RL cumulative reward (test period)", rl_vs_rule_fig))
...
#
    ...
    # --- Stubs to keep downstream callers happy ---
    import pandas as _pd_stub
    horizon_df = _pd_stub.DataFrame()
    action_summary = None
    action_rho = None
    action_aligned = None
    best_h = None
...
#
    ...
    rule_pnl_test = out_rule["reward"][out_rule["reward"].index > pd.Timestamp(cfg.qlearning.train_end)].copy()
    rl_pnl_test = out_rl["test_pnl"].copy()
    common_idx = rule_pnl_test.index.intersection(rl_pnl_test.index)
    rule_pnl_test = rule_pnl_test.loc[common_idx]
    rl_pnl_test = rl_pnl_test.loc[common_idx]
    if not rule_pnl_test.empty and not rl_pnl_test.empty:
